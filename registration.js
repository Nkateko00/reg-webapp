...
if (!regNumber == "") {
    ...
    if (/C[AYJ] \d{3,6}$/.test(regNumber) || /C[AYJ] \d{3}-\d{3}$/.test(regNumber)) {
        ...
        const newRegistration = theRegistration.rows[0].newReg

        let checking

        
        if (newRegistration > 0) {
            checking = await pool.query(`select * from places where reg_number = $1`, [regNumber])
        } else {
            return false
        }
        ...
        if (checking.rowCount === 0) {
            await pool.query(`insert into registration (reg_number, places_id) values ($1, $2)`, [regNumber, id])
        } else {
            return false
        }
